...
Term Frequency - Inverse Document Frequency (TF-IDF) is a measure of how important a word is in a document, adjusted for the fact that some words appear more frequently in general. In colloquial terms, the TF-IDF of a word is high when it appears frequently in a document but rarely in the corpus (collection of all documents).
...
top_word_tfidf |>
  ...
  facet_wrap(~subreddit, ncol = 3, scales = "fixed") +
  scale_x_reordered() +
  labs(x = NULL, 
       y = "TF-IDF",
