...
We fit two one-way Welch-ANOVA models to our dataset, one to test for statiscially significant differences between the average sentiment score of each subreddit, and one to compare the average comment counts per post. The results, available in @tbl-welch-anova-results, were conclusive. At least one statistically significant difference betweeen subreddit sentiment means (p < 0.001) @tbl-welch-anova-results-1 and average comment counts (p < 0.001) @tbl-welch-anova-results-2 was detected in both tests. Since ANOVA tests only indicate the presence of one pairwise difference in the groups you are interested in, the logical follow up was to conduct a series of post-hoc tests. For Welch's ANOVA, the standard procedure is to use the Games-Howell Test CITE, which can assess mean differences across all groups without relying on equal variance.
